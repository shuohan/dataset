...
from network_utils.data_decorators import Deformer
...
print(np.sum(data_deformer._x_deform), np.sum(mask_deformer._x_deform),
      data_deformer.order, mask_deformer.order)
...
plt.subplot(2, 3, 4)
plt.imshow(deformed_label[shape[0]//2, :, :])
plt.subplot(2, 3, 5)
plt.imshow(deformed_label[:, shape[1]//2, :])
plt.subplot(2, 3, 6)
plt.imshow(deformed_label[:, :, shape[2]//2])
print('show')
